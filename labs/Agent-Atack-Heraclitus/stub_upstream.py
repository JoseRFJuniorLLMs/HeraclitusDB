...
def hits():
    with LOCK: return HITS

class H(BaseHTTPRequestHandler):
    server_version='HeraclitusSafeMCPStub/1'
    ...
    def do_GET(self):
        if self.path=='/hits': return self.out(200,{'hits':hits()})
...
def main():
    ...
    print(f'SAFE MCP stub em http://{a.bind}:{a.port}  hits=/hits',flush=True)
